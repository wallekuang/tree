/* $Copyright: $
 * Copyright (c) 1996 - 2018 by Lucien
 * All Rights reserved
 *
...
@dev : 
*/
off_t markdown_listdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  char *path;
...
off_t markdown_rlistdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  struct _info **dir;
  off_t size = 0;
  char *err;
  ...
  markdownr_listdir(dir, d, dt, ft, lev);

  return size;
}

void  markdownr_listdir(struct _info **dir, char *d, int *dt, int *ft, u_long lev)
{
  char *path;
  long pathsize = 0;
  ...
  while(*dir) {
	...
	if (!strcmp(d,"/")) sprintf(path,"%s%s",d,(*dir)->name);
      ...
      markdownr_listdir((*dir)->child, fflag? path : NULL, dt, ft, lev+1);
      nlf = TRUE;
      *dt += 1;
    } else {
      if ((*dir)->isdir) *dt += 1;
